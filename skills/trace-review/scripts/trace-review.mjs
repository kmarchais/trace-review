...
function parseArgs(argv) {
    const first = argv[0];
    const command = first === "quick" || first === "prepare" || first === "refine" || first === "finish"
        ? first
        : "quick";
    const args = {
        command,
    ...
    const optionStart = first === "quick" || first === "prepare" || first === "refine" || first === "finish" ? 1 : 0;
...
function existingReviewFiles(directory) {
    try {
        return fs
...
function reserveUniqueReviewPath(preferredPath) {
    ...
    for (let suffix = 1;; suffix++) {
        const candidate = path.join(directory, suffix === 1 ? `${stem}${extension}` : `${stem}-${suffix}${extension}`);
        try {
...
function prepare(args, announceNext = true) {
    ...
    input.groupingContract = {
        ...
        guidance: "Preserve detector-authored repeated-change units as dedicated mechanical groups. Assign every candidate change ID exactly once. Use change-specific titles grounded in titleEvidence; never reuse classifier labels.",
    };
    ...
    input.workflow = {
        ...
        html: relative(inputPath, path.join(outputDir, reviewFileName(input))),
...
function refine(args) {
    if (!args.input)
        usage("refine requires --input");
    if (!args.rules)
        usage("refine requires --rules");
    ...
    let metrics = {};
    try {
        metrics = parseJson(fs.readFileSync(metricsPath, "utf8"));
    }
    catch { }
    writeJson(metricsPath, {
        ...metrics,
...
function finish(args) {
    ...
    console.log(reservation.existing.length
