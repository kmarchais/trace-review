...
if (args.help)
    usage();
if (!args.context)
    usage("--context requires a value");
if (!args.mode)
    usage("--mode requires a value");
if (process.argv.includes("--out") && !args.out)
...
try {
    ...
    const outputPath = path.resolve(args.out || path.join(path.dirname(contextPath), "analysis-input.json"));
    const diffPath = context.diff && typeof context.diff === "object" && context.diff.path
        ? path
            .relative(path.dirname(outputPath), path.resolve(path.dirname(contextPath), context.diff.path))
            .replaceAll("\\", "/")
        : undefined;
