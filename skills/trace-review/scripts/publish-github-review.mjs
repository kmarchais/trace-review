...
import { GithubReviewPublicationError, parseGithubReviewPlan, publishGithubReview, } from "./lib/github-review.mjs";
function parseArgs(argv) {
    const args = { confirm: false, help: false };
...
const publisher = {
    ...
    async createReview(target, request) {
        const url = gh([
            "api",
            "--method",
            "POST",
...
async function main() {
    ...
    const result = await publishGithubReview(plan, {
        ...
        async confirm(_plan, preview) {
            ...
            if (!process.stdin.isTTY) {
                throw new Error("Interactive confirmation is unavailable. Re-run with --confirm after reviewing the preview.");
            }
    ...
    console.log(`Published ${result.nativeComments} native thread${result.nativeComments === 1 ? "" : "s"} with ${result.fallbackComments} summary fallback${result.fallbackComments === 1 ? "" : "s"}.`);
    if (result.url)
        console.log(result.url);
}
main().catch((error) => {
    const message = error instanceof Error ? error.message : String(error);
    const prefix = error instanceof GithubReviewPublicationError
