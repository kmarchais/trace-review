...
export const REVIEW_MODES = Object.freeze(["workspace", "lm-analysis", "deep-audit"]);
...
const BLOCK_WIDTHS = new Set(["full", "two-thirds", "half", "third"]);
...
export function validateReviewSpec(spec, options = {}) {
    const diagnostics = [];
    const baseDir = path.resolve(options.baseDir || process.cwd());
    const checkFiles = options.checkFiles === true;
    const add = (code, fieldPath, message, hint) => {
        diagnostics.push({
            level: "error",
            code,
            path: fieldPath,
    ...
    const requireString = (value, fieldPath, { allowEmpty = false } = {}) => {
        if (typeof value !== "string" || (!allowEmpty && value.trim() === "")) {
            add("expected-string", fieldPath, `Expected ${fieldPath} to be a${allowEmpty ? "" : " non-empty"} string.`);
            return false;
        }
        return true;
    };
    const optionalString = (value, fieldPath, opts) => value === undefined || requireString(value, fieldPath, opts);
    const enumValue = (value, values, fieldPath) => {
        if (!values.has(value)) {
            add("invalid-enum", fieldPath, `Unsupported value '${String(value)}' at ${fieldPath}.`, `Use one of: ${[...values].join(", ")}.`);
            return false;
        }
        return true;
    };
    const rejectUnknown = (value, allowed, fieldPath) => {
        if (!isObject(value))
            return;
        for (const key of Object.keys(value)) {
            if (!allowed.has(key)) {
                add("unknown-field", fieldPath === "$" ? key : `${fieldPath}.${key}`, `Unknown field '${key}' at ${fieldPath}.`, "Remove the field or migrate to a schema version that defines it.");
            }
        }
    };
    if (!isObject(spec)) {
    ...
    if (spec.schemaVersion !== REVIEW_SPEC_VERSION) {
        add(spec.schemaVersion == null ? "missing-schema-version" : "unsupported-schema-version", "schemaVersion", spec.schemaVersion == null
            ? "The review specification is missing schemaVersion."
            : `Unsupported review specification schema version '${spec.schemaVersion}'.`, `Set schemaVersion to ${REVIEW_SPEC_VERSION}.`);
    }
    if (typeof spec.mode !== "string" || !REVIEW_MODES.includes(spec.mode)) {
        add(spec.mode == null ? "missing-mode" : "invalid-mode", "mode", spec.mode == null
            ? "The review specification is missing mode."
            : `Unknown review mode '${spec.mode}'.`, `Use one of: ${REVIEW_MODES.join(", ")}.`);
    }
    ...
    spec.prs.forEach((pr, index) => {
        ...
        if (hasDiff === hasDiffFile) {
            add("invalid-diff-source", root, `${root} must define exactly one of diff or diffFile.`, "Use diffFile for normal reviews and inline diff only for small fixtures.");
        }
        else if (hasDiffFile && checkFiles) {
            const diffFile = pr.diffFile;
            ...
            if (!Array.isArray(pr.blocks)) {
                ...
                pr.blocks.forEach((block, blockIndex) => {
                    ...
                    if (!BLOCK_TYPES.has(block.type)) {
                        add("invalid-block-type", `${blockPath}.type`, `Unknown summary block type '${block.type}'.`, `Use one of: ${[...BLOCK_TYPES].join(", ")}.`);
                    }
                    if (block.width !== undefined && !BLOCK_WIDTHS.has(block.width)) {
                        add("invalid-block-width", `${blockPath}.width`, `Unknown block width '${block.width}'.`, `Use one of: ${[...BLOCK_WIDTHS].join(", ")}.`);
                    }
                    if (block.type === "diagram") {
                        const sources = ["svg", "svgFile", "mermaid"].filter((key) => typeof block[key] === "string" && block[key] !== "");
                        if (sources.length !== 1)
                            add("invalid-diagram-source", blockPath, `${blockPath} must define exactly one of svg, svgFile, or mermaid.`);
                    }
        ...
        if (pr.diagrams !== undefined) {
            if (!Array.isArray(pr.diagrams)) {
                ...
                pr.diagrams.forEach((diagram, diagramIndex) => {
                    ...
                    rejectUnknown(diagram, new Set(["title", "surface", "svg", "svgFile", "mermaid"]), diagramPath);
                    const sources = ["svg", "svgFile", "mermaid"].filter((key) => typeof diagram[key] === "string" && diagram[key] !== "");
                    if (sources.length !== 1)
                        add("invalid-diagram-source", diagramPath, `${diagramPath} must define exactly one of svg, svgFile, or mermaid.`);
        ...
        if (pr.groups !== undefined) {
            if (!Array.isArray(pr.groups)) {
                ...
                pr.groups.forEach((group, groupIndex) => {
                    ...
                    rejectUnknown(group, new Set(["id", "kind", "title", "note", "collapsed", "files"]), groupPath);
                    if (requireString(group.id, `${groupPath}.id`)) {
                        if (groupIds.has(group.id))
                            add("duplicate-group-id", `${groupPath}.id`, `Group id '${group.id}' is duplicated within ${root}.`);
                        groupIds.add(group.id);
                    }
                    requireString(group.title, `${groupPath}.title`);
                    if (group.kind !== undefined)
                        enumValue(group.kind, GROUP_KINDS, `${groupPath}.kind`);
                    if (!Array.isArray(group.files) || group.files.length === 0) {
                        add("missing-group-files", `${groupPath}.files`, `${groupPath}.files must contain at least one file path.`);
                    }
                    else {
                        group.files.forEach((file, fileIndex) => {
                            const filePath = `${groupPath}.files[${fileIndex}]`;
                            if (!requireString(file, filePath))
                                return;
                            if (groupedFiles.has(file)) {
                                add("overlapping-groups", filePath, `File '${file}' is assigned to both '${groupedFiles.get(file)}' and '${group.id || groupIndex}'.`, "Assign each file to one group. Unlisted files remain visible under Other changes.");
                            }
                            else {
                                groupedFiles.set(file, typeof group.id === "string" && group.id ? group.id : String(groupIndex));
                            }
                        });
                    }
                });
        ...
        if (groupSources.length > 1) {
            add("multiple-group-sources", root, `${root} defines multiple grouping sources: ${groupSources.join(", ")}.`, "Use one of groups, groupFile, changeGroups, or autoGroups.");
        }
        if (pr.groupFile !== undefined) {
            if (requireString(pr.groupFile, `${root}.groupFile`) && checkFiles) {
                ...
                if (!fs.existsSync(groupPath)) {
                    add("group-file-not-found", `${root}.groupFile`, `Group file '${pr.groupFile}' does not exist.`, `Resolved path: ${groupPath}`);
                }
            }
        }
        if (pr.changeGroups !== undefined) {
            if (!isObject(pr.changeGroups)) {
                ...
                add("unsupported-group-schema", `${root}.changeGroups.schemaVersion`, `Unsupported change-group schema version '${pr.changeGroups.schemaVersion}'.`, "Use change-group schema version 1.");
            }
        }
        ...
        if (pr.review !== undefined) {
            if (!isObject(pr.review)) {
                ...
                if (!Array.isArray(pr.review.comments)) {
                    add("expected-array", `${root}.review.comments`, `${root}.review.comments must be an array, including when it is empty.`);
                }
                else {
                    pr.review.comments.forEach((comment, commentIndex) => {
                        ...
                        const validLine = typeof comment.line === "number" &&
                        ...
                        if (!validLine && !validOldLine) {
                            add("invalid-line-anchor", `${commentPath}.line`, "Finding lines must be positive new-file integers or old-file anchors such as 'o7'.");
                        }
                        ...
                        if (comment.options !== undefined) {
                            if (!Array.isArray(comment.options)) {
                                add("expected-array", `${commentPath}.options`, `${commentPath}.options must be an array.`);
                            }
                            else {
                                comment.options.forEach((option, optionIndex) => requireString(option, `${commentPath}.options[${optionIndex}]`));
                                if (comment.options.length < 2 ||
                                    comment.options.length > 4 ||
                                    new Set(comment.options).size !== comment.options.length) {
                                    add("invalid-finding-options", `${commentPath}.options`, `${commentPath}.options must contain 2-4 distinct choices.`);
                                }
                            }
                        }
                        optionalString(comment.suggestedChange, `${commentPath}.suggestedChange`);
                        if (typeof comment.confidence !== "number" ||
...
export function formatReviewSpecDiagnostics(validation) {
