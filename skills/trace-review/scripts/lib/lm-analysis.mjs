...
export function assessAnalysisRisk(context) {
    ...
    if (context.preflight?.patch?.valid === false)
        reasons.push("Patch validation failed.");
    if (groups.some((group) => group.risk === "high"))
        reasons.push("At least one candidate group is high risk.");
    if (files.some((file) => file.binary))
        reasons.push("The change includes binary content.");
    if (files.some((file) => file.generated))
        reasons.push("The change includes generated content.");
    if ((totals.files || 0) > 100)
        reasons.push("The change touches more than 100 files.");
    ...
    if ((totals.files || 0) > 30)
        medium.push("The change touches more than 30 files.");
...
export function prepareAnalysisInput(context, options = {}) {
    ...
    return {
        ...
        findingContract: {
            ...
            guidance: "Emit only actionable, verifiable findings. Keep body and rationale to one precise sentence each. Provide 2-4 short, finding-specific response options. Include suggestedChange only when a concrete replacement is justified. An empty findings array is valid.",
        },
    };
}
...
export function validateAnalysisResult(result, input) {
    ...
    if (Number.isInteger(maxFindings) && findings.length > maxFindings) {
        add("finding-budget-exceeded", "findings", `Focused analysis allows at most ${maxFindings} findings for this change set; received ${findings.length}.`);
    }
    findings.forEach((finding, index) => {
        const root = `findings[${index}]`;
        for (const field of Object.keys(finding || {})) {
            if (!FINDING_FIELDS.has(field)) {
                add("unknown-finding-field", `${root}.${field}`, `Finding field '${field}' is not part of the review-spec contract.`);
            }
        }
        if (typeof finding?.file !== "string" || !finding.file.trim()) {
            add("missing-finding-field", `${root}.file`, "A finding must identify a file.");
        }
        const validLine = (typeof finding?.line === "number" && Number.isInteger(finding.line) && finding.line > 0) ||
        ...
        if (!validLine) {
            ...
            const anchored = (input?.facts?.changeGroups?.inventory || []).some((change) => change.file === finding.file &&
                change[rangeKey] &&
                change[rangeKey].count !== 0 &&
                line >= change[rangeKey].start &&
                line <= change[rangeKey].end);
            if (!anchored) {
                add("anchor-not-in-diff", `${root}.line`, `Finding anchor '${finding.file}:${finding.line}' is not inside a changed hunk range.`);
            }
        }
        ...
        if (typeof finding?.rationale !== "string" || !finding.rationale.trim()) {
            ...
            add("finding-too-long", `${root}.rationale`, "Keep the rationale to 180 characters or fewer.");
        }
        if (!Array.isArray(finding?.options) ||
            finding.options.length < 2 ||
            finding.options.length > 4 ||
            finding.options.some((option) => typeof option !== "string" || !option.trim() || option.length > 64) ||
            new Set(finding.options.map((option) => option.trim().toLowerCase())).size !==
                finding.options.length) {
            add("invalid-finding-options", `${root}.options`, "Provide 2-4 distinct, non-empty response options of at most 64 characters each.");
        }
        if (finding?.suggestedChange !== undefined &&
            (typeof finding.suggestedChange !== "string" || !finding.suggestedChange.trim())) {
            add("invalid-suggested-change", `${root}.suggestedChange`, "A suggested change must be a non-empty code replacement.");
        }
    });
