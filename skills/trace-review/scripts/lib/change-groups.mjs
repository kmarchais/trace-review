...
function includeSignatures(change) {
    return [
        ...new Set(change.added.map((line) => line.trim()).filter((line) => /^#\s*include\b/.test(line))),
    ];
}
...
export function detectChangeGroups(text, preflight = {}) {
    ...
    for (const [signature, candidates] of repeated) {
        ...
        add(`Repeat ${signature}`, "mechanical", "Apply the same dependency include wherever the changed code needs it.", "low", 0.98, [
            "Confirm every changed unit needs the include.",
            "Check that no include introduces an ordering or platform dependency.",
        ], unique, [`The exact added line '${signature}' repeats in ${unique.length} change units.`]);
    }
