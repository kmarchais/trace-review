...
const GENERIC_TITLE_RE = /^(definitions?|consumers?|definitions? and consumers?|associated tests?|configuration and build integration|needs inspection|unclassified|imports? and includes?|formatting-only changes?|generated dependency locks?|renames? without content changes?)$/i;
...
export function validateLmGroupingResult(result, candidates) {
    ...
    const repeatedPattern = (changeId) => {
    ...
    for (const [index, group] of groups.entries()) {
        ...
        if (!String(group?.intent || "").trim() ||
            ...
            diagnostics.push({
                ...
                group: groupLabel,
                message: "Every group needs intent, low/medium/high risk, confidence from 0 to 1, evidence, and reviewer checks.",
            });
        }
        ...
        if (!group?.titleEvidence ||
            ...
            diagnostics.push({
                ...
                group: groupLabel,
                message: "Every group title must cite assigned change IDs and explain how the title is grounded in those changes.",
            });
        }
        else {
        ...
        const repeatedPatterns = new Set(changeIds.map(repeatedPattern).filter((pattern) => pattern !== null));
        ...
        if (repeatedPatterns.size > 1 || (repeatedPatterns.size === 1 && includesResidual)) {
            diagnostics.push({
                ...
                group: groupLabel,
                message: "A deterministic repeated-change pattern must remain in its own dedicated mechanical group.",
            });
        }
        for (const pattern of repeatedPatterns) {
            const owners = repeatedAssignments.get(pattern) ?? new Set();
...
export function finalizeLmGrouping(result, candidates) {
    ...
    const groupIds = new Map(result.groups.map((group, index) => [group.title.toLowerCase(), `g${index + 1}`]));
    const groups = result.groups.map((group, index) => ({
        ...
        changes: (group.changeIds ?? []).map((changeId) => {
            ...
            if (!change)
                throw new Error(`Validated change '${changeId}' is missing.`);
            return change;
        }),
    }));
    const explicitEdges = result.groups.flatMap((group) => (group.readAfter || []).map((prerequisite) => {
        const from = groupIds.get(prerequisite.toLowerCase());
        const to = groupIds.get(group.title.toLowerCase());
        if (!from || !to)
            throw new Error("Validated group dependency is missing.");
        return {
            from,
            to,
    ...
    for (const edge of candidates.dependencyGraph?.edges || []) {
        for (const from of finalGroupsForCandidate(edge.from)) {
            for (const to of finalGroupsForCandidate(edge.to)) {
                if (from !== to)
                    inferredEdges.push({ ...edge, from, to });
            }
        }
    }
