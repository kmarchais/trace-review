...
const run = (command, commandArgs, options = {}) => {
    ...
    if (check.error)
        throw new Error(`Could not run Git patch check: ${check.error.message}`);
    if (check.status !== 0) {
        throw new Error((check.stderr || "").trim() || `Git rejected the patch with exit code ${check.status}.`);
    }
    return check.stdout;
};
