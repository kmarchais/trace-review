...
const BLOCK_TYPES = new Set(["prose", "diagram", "stats", "table", "callout", "heading"]);
const BLOCK_WIDTHS = new Set(["full", "two-thirds", "half", "third"]);
const GROUP_KINDS = new Set(["mechanical", "refactor", "feature", "fix", "test", "docs", "other"]);
const SEVERITIES = new Set(["nit", "suggestion", "concern", "question", "praise", "comment"]);
...
export function validateReviewSpec(spec, options = {}) {
  ...
  const optionalString = (value, fieldPath, opts) =>
    value === undefined || requireString(value, fieldPath, opts);
  ...
  if (!REVIEW_MODES.includes(spec.mode)) {
    add(
      spec.mode == null ? "missing-mode" : "invalid-mode",
      "mode",
      spec.mode == null ? "The review specification is missing mode." : `Unknown review mode '${spec.mode}'.`,
  ...
  spec.prs.forEach((pr, index) => {
    ...
    if (pr.id) {
      if (ids.has(pr.id)) add("duplicate-pr-id", `${root}.id`, `Pull request id '${pr.id}' is duplicated.`);
      ids.add(pr.id);
    }
    ...
    if (hasDiff === hasDiffFile) {
      ...
      const diffPath = path.isAbsolute(pr.diffFile) ? pr.diffFile : path.resolve(baseDir, pr.diffFile);
    ...
    if (pr.blocks !== undefined) {
      if (!Array.isArray(pr.blocks)) {
        ...
        pr.blocks.forEach((block, blockIndex) => {
          ...
          rejectUnknown(block, new Set(["type", "width", "md", "text", "level", "title", "variant", "surface", "svg", "svgFile", "mermaid", "items", "headers", "rows"]), blockPath);
          ...
          if (block.type === "diagram") {
            ...
            if (sources.length !== 1) add("invalid-diagram-source", blockPath, `${blockPath} must define exactly one of svg, svgFile, or mermaid.`);
          }
          if (block.type === "stats" && !Array.isArray(block.items)) add("expected-array", `${blockPath}.items`, `${blockPath}.items must be an array.`);
          if (block.type === "table" && !Array.isArray(block.rows)) add("expected-array", `${blockPath}.rows`, `${blockPath}.rows must be an array.`);
        });
      }
    }
    ...
    if (pr.diagrams !== undefined) {
      if (!Array.isArray(pr.diagrams)) {
        ...
        pr.diagrams.forEach((diagram, diagramIndex) => {
          ...
          if (sources.length !== 1) add("invalid-diagram-source", diagramPath, `${diagramPath} must define exactly one of svg, svgFile, or mermaid.`);
          if (diagram.surface !== undefined) enumValue(diagram.surface, new Set(["light", "dark"]), `${diagramPath}.surface`);
        });
      }
    }
      ...
      if (!Array.isArray(pr.groups)) {
        ...
        pr.groups.forEach((group, groupIndex) => {
          ...
          if (requireString(group.id, `${groupPath}.id`)) {
            if (groupIds.has(group.id)) add("duplicate-group-id", `${groupPath}.id`, `Group id '${group.id}' is duplicated within ${root}.`);
          ...
          if (!Array.isArray(group.files) || group.files.length === 0) {
            ...
            group.files.forEach((file, fileIndex) => {
              ...
              if (groupedFiles.has(file)) {
                ...
                groupedFiles.set(file, group.id || String(groupIndex));
              }
            });
          }
        });
    ...
    if (pr.review !== undefined) {
      if (!isObject(pr.review)) {
        ...
        if (pr.review.verdict !== undefined) enumValue(pr.review.verdict, VERDICTS, `${root}.review.verdict`);
