...
export interface RepeatedChangePattern {
  id: string;
  kind: "exact" | "parameterized" | "composite" | "rule";
  title: string;
  signature: string;
  support: number;
  files: string[];
...
export interface RepeatedChangeDiscovery {
  schemaVersion: 1;
  atoms: EditAtom[];
  patterns: RepeatedChangePattern[];
  assignments: Record<string, string>;
  unmatchedAtomIds: string[];
  validation: {
    valid: boolean;
    diagnostics: Array<{ code: string; message: string; atomId?: string }>;
  };
}

export interface RepeatedChangeOptions {
  minimumFiles?: number;
  automatic?: boolean;
  rules?: DetectorRule[];
}

export interface DetectorRuleOperation {
  side: EditSide;
  pattern: string;
...
export interface DetectorRule {
  ...
  title: string;
  minimumFiles?: number;
  operations: DetectorRuleOperation[];
}
...
export function parseDetectorRuleSet(value: unknown): DetectorRuleSet {
  const root = objectValue(value, "Detector rule set");
  if (root.schemaVersion !== 1) throw new Error("Detector rule set schemaVersion must be 1.");
  if (!Array.isArray(root.rules)) throw new Error("Detector rule set rules must be an array.");
  if (root.rules.length > 64) throw new Error("Detector rule sets cannot exceed 64 rules.");
  const ids = new Set<string>();
  const rules = root.rules.map((candidate, ruleIndex): DetectorRule => {
    ...
    if (ids.has(id)) throw new Error(`Detector rule id '${id}' is duplicated.`);
    ids.add(id);
    if (!title) throw new Error(`rules[${ruleIndex}].title is required.`);
    ...
    const operations = rule.operations.map((candidateOperation, operationIndex) => {
      const operation = objectValue(
        candidateOperation,
        `rules[${ruleIndex}].operations[${operationIndex}]`,
      );
      if (operation.side !== "add" && operation.side !== "delete") {
        throw new Error(
          `rules[${ruleIndex}].operations[${operationIndex}].side must be add or delete.`,
        );
      }
      const side: EditSide = operation.side;
      ...
      if (
        ...
        throw new Error(
          `rules[${ruleIndex}].operations[${operationIndex}].location must be start, end, or any.`,
        );
      }
      return {
        side,
...
function normalizedExact(atom: EditAtom): string | null {
  const text = atom.text.trim();
  if (!text) return null;
  return `${atom.side}\u0000${text}`;
}

function lowInformationExact(signature: string): boolean {
  return /^[{}()[\],;:.\-+*/]+$/.test(signature.split("\u0000")[1] || "");
}

interface Token {
  value: string;
  type: "identifier" | "number" | "string" | "punctuation";
}

function tokenizeForPattern(text: string): Token[] {
  const values =
    text.match(/"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[A-Za-z_$][\w$]*|\d+(?:\.\d+)?|[^\s]/g) || [];
...
function tokenShape(atom: EditAtom): { key: string; tokens: Token[] } | null {
...
function parameterizedPattern(candidates: readonly EditAtom[]): {
  signature: string;
  bindings: Map<string, Record<string, string>>;
  specificity: number;
} | null {
  const rows = candidates.map((atom) => tokenizeForPattern(atom.text));
  if (!rows.length || rows.some((tokens) => tokens.length !== rows[0].length)) return null;
  const pattern: string[] = [];
  const variablePositions: Array<{ index: number; name: string }> = [];
  ...
  for (let index = 0; index < rows[0].length; index++) {
    ...
    if (values.every((value) => value === values[0])) {
      ...
      if (rows[0][index].type === "identifier") stableIdentifiers++;
      continue;
    }
    const types = rows.map((tokens) => tokens[index].type);
    if (!types.every((type) => type === types[0]) || types[0] === "punctuation") return null;
    const name = `${types[0]}${++serial}`;
    pattern.push(`$${name}`);
    variablePositions.push({ index, name });
  }
  if (!variablePositions.length || stableIdentifiers === 0) return null;
  const bindings = new Map<string, Record<string, string>>();
  candidates.forEach((atom, rowIndex) => {
    bindings.set(
      atom.id,
      Object.fromEntries(
        variablePositions.map(({ index, name }) => [name, rows[rowIndex][index].value]),
      ),
    );
  });
  return {
...
function titleForExact(signature: string): string {
...
function matchesLocation(
  atom: EditAtom,
  location: DetectorRuleOperation["location"],
  lastHunk: number,
): boolean {
  if (!location || location === "any") return true;
  if (location === "start") return atom.line <= 12;
...
function matchRulePattern(
  ...
  for (let index = 0; index < expected.length; index++) {
    ...
    if (/^\$[A-Za-z_]\w*$/.test(wanted)) {
      ...
      if (next[name] !== undefined && next[name] !== received) return null;
      next[name] = received;
...
function ruleOccurrence(
  atoms: readonly EditAtom[],
  rule: DetectorRule,
): { atomIds: string[]; bindings: Record<string, string> } | null {
  ...
  const search = (
    ...
    if (steps > 50_000) {
      throw new Error(
        `Detector rule '${rule.id}' exceeded its evaluation budget in '${atoms[0]?.file || "unknown file"}'.`,
      );
    }
    ...
    for (const atom of atoms) {
      ...
      if (tail) return { atomIds: [atom.id, ...tail.atomIds], bindings: tail.bindings };
    }
    return null;
  };
...
function applyRules(
  ...
  assignments: Record<string, string>,
): RepeatedChangePattern[] {
  const byFile = new Map<string, EditAtom[]>();
  for (const atom of atoms) {
    if (atom.fileStatus !== "modified") continue;
    const rows = byFile.get(atom.file) ?? [];
    rows.push(atom);
    byFile.set(atom.file, rows);
  }
  const patterns: RepeatedChangePattern[] = [];
  for (const rule of rules) {
    if (!rule.id.trim() || !rule.title.trim() || !rule.operations.length) continue;
    ...
    for (const [file, fileAtoms] of byFile) {
      ...
      if (occurrence) occurrences.push({ file, ...occurrence });
    }
    const minimumFiles = Math.max(2, rule.minimumFiles ?? 2);
    if (occurrences.length < minimumFiles) continue;
...
function mergeRepeatedReplacements(
  patterns: readonly RepeatedChangePattern[],
  atoms: readonly EditAtom[],
  assignments: Record<string, string>,
): RepeatedChangePattern[] {
  const atomById = new Map(atoms.map((atom) => [atom.id, atom]));
  const consumed = new Set<string>();
  const merged: RepeatedChangePattern[] = [];
  const sameFiles = (left: RepeatedChangePattern, right: RepeatedChangePattern): boolean =>
    left.files.length === right.files.length &&
    left.files.every((file, index) => file === right.files[index]);
  const sideOf = (pattern: RepeatedChangePattern): EditSide | null => {
    const side = pattern.signature.split("\u0000")[0];
    return side === "add" || side === "delete" ? side : null;
  };
  const occurrenceFor = (
    pattern: RepeatedChangePattern,
    file: string,
  ): PatternOccurrence | undefined =>
    pattern.occurrences.find((occurrence) => occurrence.file === file);
  const replacementKey = (pattern: RepeatedChangePattern): string | null => {
  ...
  const additionsByKey = new Map<string, RepeatedChangePattern[]>();
  for (const pattern of patterns) {
    if (pattern.kind === "rule" || sideOf(pattern) !== "add") continue;
  ...
  for (const deletion of patterns) {
    ...
    const addition = (additionsByKey.get(key) || []).find((candidate) => {
      ...
      return deletion.files.every((file) => {
        ...
        if (!deletedAtom || !addedAtom || deletedAtom.hunk !== addedAtom.hunk) return false;
    ...
    const pattern: RepeatedChangePattern = {
      id,
      kind: "composite",
...
export function validateRepeatedChangeDiscovery(
  discovery: Pick<
    RepeatedChangeDiscovery,
    "atoms" | "patterns" | "assignments" | "unmatchedAtomIds"
  >,
): RepeatedChangeDiscovery["validation"] {
  const { atoms, patterns, assignments, unmatchedAtomIds } = discovery;
  const diagnostics: RepeatedChangeDiscovery["validation"]["diagnostics"] = [];
  ...
  for (const pattern of patterns) {
    ...
    if (
      pattern.support !== pattern.files.length ||
  ...
  if (
    expectedUnmatched.length !== unmatchedAtomIds.length ||
...
export function discoverRepeatedChanges(
  patch: string,
  options: RepeatedChangeOptions = {},
): RepeatedChangeDiscovery {
  const minimumFiles = Math.max(2, options.minimumFiles ?? 2);
  const atoms = parseEditAtoms(patch);
  const assignments: Record<string, string> = {};
  const patterns: RepeatedChangePattern[] = applyRules(atoms, options.rules || [], assignments);
  if (options.automatic === false) {
    const result: RepeatedChangeDiscovery = {
      schemaVersion: 1,
      atoms,
      patterns,
      assignments,
  ...
  const bySignature = new Map<string, EditAtom[]>();
  for (const atom of atoms) {
    if (atom.fileStatus !== "modified") continue;
    if (assignments[atom.id] !== undefined) continue;
  ...
  for (const [signature, candidates] of bySignature) {
    ...
    if (files.length < requiredFiles) continue;
    ...
    for (const atom of candidates) assignments[atom.id] = id;
  }

  const byShape = new Map<string, EditAtom[]>();
  for (const atom of atoms) {
    if (atom.fileStatus !== "modified") continue;
    if (assignments[atom.id] !== undefined) continue;
  ...
  for (const shapeCandidates of byShape.values()) {
    const candidateSets = new Map<string, EditAtom[]>();
    const addCandidateSet = (candidates: EditAtom[]): void => {
    ...
    const byIdentifierAnchor = new Map<string, EditAtom[]>();
    ...
    const ranked = [...candidateSets.values()]
      ...
      .filter(
        (
          ...
          candidates: EditAtom[];
          parameterized: NonNullable<ReturnType<typeof parameterizedPattern>>;
          files: string[];
        } => candidate.parameterized !== null && candidate.files.length >= minimumFiles,
      )
      .sort(
        (left, right) =>
          right.parameterized.specificity - left.parameterized.specificity ||
          right.files.length - left.files.length,
      );
    for (const rankedCandidate of ranked) {
      const candidates = rankedCandidate.candidates.filter(
        (atom) => assignments[atom.id] === undefined,
      );
      const files = [...new Set(candidates.map((atom) => atom.file))];
      if (files.length < minimumFiles) continue;
      ...
      const occurrences = files.map((file) => {
        ...
        return {
          ...
          bindings: Object.assign(
            {},
            ...matchingAtoms.map((atom) => parameterized.bindings.get(atom.id) || {}),
          ),
        };
      });
      patterns.push({
